...
1. `asset_host` 已是新的服务器资产主表
2. `xui_instance` 现在优先绑定 `asset_id`，`host_label` 只是兼容字段
3. `forward` 已支持 `remote_source_*`，可以记录转发远端来自哪个 X-UI 节点
4. 前端已经有单独的 `服务器资产` 页面，顶栏可直接进入
5. “转发管理”已支持管理员从 X-UI 同步节点中直接选远端地址

如果后续继续推进多项目整合，必须优先围绕 `asset_host` 扩展，而不是继续在各模块里各自维护一套“服务器标签”。
