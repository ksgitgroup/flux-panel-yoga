...
if [ "$TAG" = "local" ] && [ "$(uname -s)" = "Darwin" ]; then
    ...
    echo "---------------------------------------------------"
    docker build -f springboot-backend/Dockerfile.local -t "$REGISTRY/springboot-backend:$TAG" ./springboot-backend

    echo "---------------------------------------------------"
    echo "正在构建前端镜像: $REGISTRY/vite-frontend:$TAG"
    echo "---------------------------------------------------"
    docker build -f vite-frontend/Dockerfile.local -t "$REGISTRY/vite-frontend:$TAG" ./vite-frontend
